...
init python:
                    ...
                    if item1.visible == True:
                        if item1.x <= x <= item1.x + item1.width and item1.y <= y <= item1.y + item1.height:
                            ...
                            if i_combine == False and ie_combine == False:
                                ...
                                item1.y = item1.original_y
                                item1.zorder = 0
                                if item1.type == "evidence_bag":
                                    renpy.hide_screen("packed_fingerprint")
                                    renpy.hide_screen("packing_swab")
                                    renpy.hide_screen("collecting_dish_towel")
                                    renpy.hide_screen("collected_knife")
                                    if not currently_collecting_fingerprint:
                                        renpy.jump("select_screen")
                                elif item1.type == "tamper_tape":
                                    renpy.jump("put_away_tamper_tape")
                    ...
                    if item1.visible == True:
                        if item1.x <= x <= item1.x + item1.width and item1.y <= y <= item1.y + item1.height:
                            ...
                            if i_combine == False and ie_combine == False:
                                ...
                                item1.zorder = 0
                                # -------- ADDED CODE ---------
                                if item1.type == "evidence_markers":
                                    renpy.hide_screen("place_marker_dish_towel")
                                    renpy.hide_screen("place_marker_stove")
                                    renpy.hide_screen("place_marker_kitchen")
                                    if not currently_collecting_fingerprint:
                                        renpy.jump("select_screen")
                                elif item1.type == "black_granular_powder":
                                    renpy.hide_screen("dust_fingerprint")
                                elif item1.type == "scalebar":
                                    renpy.hide_screen("scalebar_screen")
                                elif item1.type == "lifting_tape":
                                    renpy.hide_screen("taping_screen")
                                elif item1.type == "backing_cards":
                                    renpy.hide_screen("placing_backing_card")
                                elif item1.type == "forensic_tube":
                                    renpy.hide_screen("collected_knife")
                                    if at_stove and not collected_knife and not put_knife_in_tube and not put_knife_in_bag and not knife_marked and not took_photo_knife_close:
                                        renpy.show_screen("knife_unmarked_camera")
                                    elif at_stove and knife_marked and not collected_knife and not put_knife_in_tube and not put_knife_in_bag and not took_photo_knife_close_marked:
                                        renpy.show_screen("knife_marked_camera")
                                    
                                    if at_stove and not took_photo_untouched_stove and not knife_marked and not fingerprint_marked and not collected_fingerprint and not collected_knife and not put_knife_in_tube and not put_knife_in_bag:
                                        renpy.show_screen("camera_screen")
                                elif item1.type == "distilled_water":
                                    selected_water = False
                                elif item1.type == "ethanol":
                                    ethanol_selected = False
                                elif item1.type == "phenolphthalein":
                                    phenolphthalein_selected = False
                                elif item1.type == "hydrogen_peroxide":
                                    hydrogen_peroxide_selected = False
                                elif item1.type == "spray_bottle":
                                    spray_bottle_selected = False
                                    currently_spraying_luminol = False
                                    renpy.hide_screen("luminol")
                                elif item1.type == "luminol_packet":
    ...
    def startDrag(item):
        ...
        if item.type == "evidence_bag":
            if currently_in_swabbing_process and not put_swab_in_tube:
                renpy.jump("busy_swabbing")
            if currently_spraying_luminol:
                renpy.jump("busy_luminol")
            if holding_swab:
                renpy.jump("busy_holding_swab")
            if currently_collecting_fingerprint and put_print_on_card and not collected_fingerprint:
                renpy.show_screen("packed_fingerprint")
            elif viewing_floor:
                if taped_swab_tube and not put_swab_in_bag and not collected_swab:
                    renpy.hide("taped_sample")
                    renpy.show_screen("packing_swab")
            elif viewing_dish_towel:
                renpy.hide_screen("dish_towel_camera")
                if not collected_dish_towel:
                    renpy.show_screen("collecting_dish_towel")
            elif at_stove:
                if knife_tube_sealed and not put_knife_in_bag:
                    renpy.hide("taped_tube_idle")
                    renpy.show_screen("collect_knife_tube")
        elif item.type == "tamper_tape":
            if currently_in_swabbing_process and not put_swab_in_tube:
                renpy.jump("busy_swabbing")
            if currently_spraying_luminol:
                renpy.jump("busy_luminol")
            ...
            if viewing_floor:
                if put_swab_in_tube and not taped_swab_tube and not put_swab_in_bag:
                    renpy.show_screen("taping_swab")
                elif put_swab_in_bag:
                    renpy.show_screen("collected_evidence")
            elif viewing_dish_towel:
                if put_towel_in_bag:
                    renpy.show_screen("collected_evidence")
                ...
                if put_knife_in_bag:
                    renpy.hide("packed_evidence_bag")
                    renpy.hide_screen("collect_knife_tube")
                    renpy.show_screen("collected_evidence")
                elif knife_not_ready_to_bag and put_knife_in_tube:
                    renpy.hide("filled_tube_idle")
                    renpy.show_screen("tube_with_knife", sealed_tube_top, sealed_tube_bottom)
        elif item.type == "camera":
            renpy.jump("display_images")
        # ---- END OF ADDED CODE ----
        inventory_drag = True
    ...
    def startDrag2(item):
        ...
        global item_dragged
        # ---- ADDED CODE ----
        global currently_spraying_luminol
        global selected_water
        global ethanol_selected
        global phenolphthalein_selected
        global hydrogen_peroxide_selected
        global spray_bottle_selected
        global currently_spraying_luminol
        global selected_luminol_packet
        item.original_x = item.x
        item.original_y = item.y
        # Check what item has been clicked on
        if item.type == "evidence_markers": # If the player has selected the evidence markers
            if currently_collecting_knife:
                renpy.jump("collecting_knife_on_stove")
            if viewing_dish_towel:
                if not collected_dish_towel and not dish_towel_marked:
                    renpy.show_screen("place_marker_dish_towel")
            elif at_stove:
                if not collected_knife and not knife_marked and not currently_collecting_fingerprint:
                    renpy.hide_screen("knife_unmarked_camera")
                    renpy.show_screen("place_marker_stove")
            elif in_kitchen:
                if not collected_dish_towel and not dish_towel_marked:
                    renpy.hide_screen("kitchen_screen")
                    renpy.show_screen("place_marker_kitchen")
            else:
                renpy.show_screen("nothing_to_mark")
        elif item.type == "black_granular_powder":
            if currently_collecting_knife:
                renpy.jump("collecting_knife_on_stove")
            if not collected_fingerprint and discovered_fingerprint:
                if dusted_fingerprint:
                    renpy.jump("has_been_dusted")
                else:
                    renpy.show_screen("dust_fingerprint")
            else:
                renpy.jump("find_fingerprint_first")
        elif item.type == "scalebar":
            if currently_collecting_knife:
                renpy.jump("collecting_knife_on_stove")
            if not collected_fingerprint and dusted_fingerprint:
                if scaled_fingerprint:
                    renpy.jump("has_been_scaled")
                else:
                    renpy.show_screen("scalebar_screen")
        elif item.type == "lifting_tape":
            if currently_collecting_knife:
                renpy.jump("collecting_knife_on_stove")
            if not collected_fingerprint and scaled_fingerprint:
                if taped_fingerprint:
                    renpy.jump("has_been_taped")
                else:
                    renpy.show_screen("taping_screen")
        elif item.type == "backing_cards":
            if currently_collecting_knife:
                renpy.jump("collecting_knife_on_stove")
            if not collected_fingerprint and taped_fingerprint:
                if put_print_on_card:
                    renpy.jump("has_been_put_on_card")
                else:
                    renpy.show_screen("placing_backing_card")
        elif item.type == "forensic_tube":
            if at_stove:
                renpy.hide_screen("knife_unmarked_camera")
                renpy.hide_screen("knife_marked_camera")
                renpy.hide_screen("camera_screen")
                if not currently_collecting_fingerprint and not put_knife_in_tube:
                    renpy.show_screen("collected_knife") 
        elif item.type == "cotton_swab":
            if viewing_floor:
                if put_swab_in_tube or put_swab_in_bag:
                    renpy.jump("already_collecting_sample")
                if currently_spraying_luminol:
                    renpy.jump("busy_luminol")
            ...
            if holding_swab:
                renpy.jump("busy_holding_swab")
            renpy.jump("ethanol")
        elif item.type == "phenolphthalein":
            if put_swab_in_tube or put_swab_in_bag:
                renpy.jump("already_collecting_sample")
            if currently_spraying_luminol:
                renpy.jump("busy_luminol")
            if holding_swab:
                renpy.jump("busy_holding_swab")
            renpy.jump("phenolphthalein")
        elif item.type == "hydrogen_peroxide":
            if put_swab_in_tube or put_swab_in_bag:
                renpy.jump("already_collecting_sample")
            if currently_spraying_luminol:
                renpy.jump("busy_luminol")
            if holding_swab:
                renpy.jump("busy_holding_swab")
            renpy.jump("hydrogen_peroxide")
        elif item.type == "spray_bottle":
            if not viewing_floor:
                renpy.jump("cannot_use_here")
            if put_swab_in_tube or put_swab_in_bag:
                renpy.jump("already_collecting_sample")
            if currently_in_swabbing_process:
                renpy.jump("busy_swabbing")
            if holding_swab:
                renpy.jump("busy_holding_swab")
            if not collected_swab:
                renpy.jump("get_sample")
            ...
            if spray_bottle_selected:
                ...
                renpy.jump("start_luminol_process")
        elif item.type == "luminol_packet":
            if put_swab_in_tube or put_swab_in_bag:
                renpy.jump("already_collecting_sample")
            if currently_in_swabbing_process:
                renpy.jump("busy_swabbing")
    ...
    def startDrag3(item):
        ...
        item.original_x = item.x
        item.original_y = item.y
        if item.type == "uva_flashlight":
            if at_stove or viewing_floor or viewing_dish_towel:
                renpy.jump("dont_need")
            else:
                renpy.jump("selected_uva")
        elif item.type == "415nm_flashlight":
            if at_stove or viewing_floor or viewing_dish_towel:
                renpy.jump("dont_need")
            else:
                renpy.jump("selected_415nm")
        elif item.type == "450nm_flashlight":
            if at_stove or viewing_floor or viewing_dish_towel:
                renpy.jump("dont_need")
            else:
                renpy.jump("selected_450nm")
        elif item.type == "530nm_flashlight":
            if at_stove or viewing_floor or viewing_dish_towel:
                renpy.jump("dont_need")
            else:
                renpy.jump("selected_530nm")
        elif item.type == "uv_flashlight":
            using_uv = False
            if viewing_dish_towel or viewing_floor:
                renpy.jump("no_fingerprints")
            elif at_stove:
                if currently_collecting_fingerprint:
                    renpy.jump("taking_fingerprint")
                elif currently_collecting_knife:
                    renpy.jump("collecting_knife_on_stove")
                else:
                    renpy.jump("uv_light")
            else:
            ...
            for i, item in enumerate(inventory_sprites):
                ...
                if button == "down" and inventory_db_enabled == True:
                    ...
                    if inventory_sprites[-1].visible == False:
                        ...
                        item.y -= toolbox_slot_size[0] + inventory_slot_padding
                        item.origial_y = item.y # added code by me
                        citem = item
                elif button == "up" and inventory_ub_enabled == True:
                    if inventory_sprites[0].visible == False:
                        reversed_index = (len(inventory_sprites) - 1) - i
                        inventory_sprites[reversed_index].y += toolbox_slot_size[0] + inventory_slot_padding
                        item.origial_y = item.y # added code by me
            ...
            for i, item in enumerate(toolbox_sprites):
                ...
                if button == "down" and toolbox_db_enabled == True:
                    ...
                    if toolbox_sprites[-1].visible == False:
                        ...
                        item.y -= toolbox_slot_size[0] + toolbox_slot_padding # changed by me
                        item.origial_y = item.y # added code by me
                    ...
                    if toolbox_sprites[0].visible == False:
                        ...
                        toolbox_sprites[reversed_index].y += toolbox_slot_size[0] + toolbox_slot_padding # changed by me
                        item.origial_y = item.y # added code by me
            ...
            for i, item in enumerate(toolboxpop_sprites):
                ...
                if button == "down" and toolboxpop_db_enabled == True:
                    ...
                    if toolboxpop_sprites[-1].visible == False:
                        # shift toolbox items up
                        item.y -= toolbox_slot_size[0] + toolboxpop_slot_padding
                        citem = item
                    ...
                    if toolboxpop_sprites[0].visible == False:
                        ...
                        toolboxpop_sprites[reversed_index].y += toolbox_slot_size[0] + toolboxpop_slot_padding
