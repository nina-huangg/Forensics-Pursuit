    ...
    def toolbox_actions(item: str) -> None:
        ...
        if item == "uv_light" and tools["uv light"]:
            ...
            if analyzing["counter"]:
                renpy.show_screen("dark_overlay_counter")
            elif analyzing["cabinet"]:
                renpy.hide_screen("back_button_overlay")
                renpy.show_screen("dark_overlay_cabinet")

        elif item == "magnetic_powder" and tools["magnetic powder"]:
            tools["magnetic powder"] = False
            hide_all_inventory()
            if analyzing["handprint"]:
                tools["gel lifter"] = True
                renpy.jump("handprint_dusted")
            else: # analyzing fingerprint
                tools["scalebar"] = True
        ...
        # Packaging -------------------------------------------
            ...
            if tools["packaging"]:
                ...
                if analyzing["fingerprint"] or analyzing["handprint"] or analyzing["carpet_cut"] or analyzing["steroids"] or analyzing["pillbox"] or analyzing["needle"] or analyzing["tylenol"] or analyzing["lower_cabinet"]:
                    renpy.jump("packaging_1")
                elif analyzing["drip"] or analyzing["carpet_stain"]:
                    renpy.jump("drip_packaging_1")
        elif item == "tube" and tools["tube"]:
            tools["tube"] = False
            hide_all_inventory()
            tools["bag"] = True
            if analyzing["drip"] or analyzing["carpet_stain"]:
                renpy.jump("drip_packaging_0")
...
# toolbox:
screen toolbox:
    # ADDED: to allow for hover selection while having the toolbar open
    showif not packaging:
        showif analyzing["cabinet"] and not analyzing["fingerprint"] and not (analyzed["tylenol"] or analyzing["tylenol"]):
            imagemap:
                idle "cabinet idle"
                hover "cabinet hover"

                # tylenol pick up
                hotspot (538, 362, 302, 294) action [SetDict(tools, "uv light", True), Jump("tylenol")] mouse "hover" 

                # vitamins inspect
                hotspot (836, 217, 790, 420) action [SetDict(tools, "uv light", True), Jump("vitamins")] mouse "hover"

        showif analyzing["cabinet"] and not analyzing["fingerprint"] and (analyzed["tylenol"] or analyzing["tylenol"]): 
            imagemap:
                idle "cabinet no tylenol idle"
                hover "cabinet no tylenol hover"
