...
init python:
    ...
    def toolbox_actions(item: str) -> None:
        ...
        if location == "internal":
            ...
            hide_all_inventory()
            if item == "syringe":
                default_mouse = "syringe"
                hide_all_inventory()
            if item == "scalpel":
                default_mouse = "scalpel"
                hide_all_inventory()
            if item == "rib_cutter":
                default_mouse = "rib cutter"
                hide_all_inventory()
            if item == "clamps":
                default_mouse = "clamps"
                hide_all_inventory()
            if item == "falcon_tube":
                default_mouse = "falcon tube"
                hide_all_inventory()
            if item == "bone_saw":
                default_mouse = "bone saw"
                hide_all_inventory()
            if item == "weight":
                default_mouse = "weight"
                hide_all_inventory()
        else:
            renpy.notify("That tool cannot be used in this step.")
    ...
    """
    Removes inventory item from the toolbox bar.

    Takes in the item name as str to be removed as a parameter.
    """
    
    def removeInventoryItemByName(item_name: str):
        for item in inventory_sprites:
...
screen full_inventory:
    ...
    imagebutton auto "UI/open-inv-%s.png" action If(renpy.get_screen("toolbox") == None, true= [Show("inv_buttons"), Show("close_inv"), Show("toolbox"), Hide("inventory"), Hide("toolboxpop")], false= [Hide("close_inv"), Hide("inv_buttons"), Hide("toolbox"), Hide("toolboxpop")]) xpos 0 ypos 0.36
...
# evidence box photo album item menu
...
screen inspectItem(items):
    ...
    button:
        ...
        $ item_name = next((name for name in inventory_item_names if name.replace(" ", "-").lower() == items[0] or name.replace(" ", "_").lower() == items[0]), "")
        
        image "Items Pop Up/{}-pop-up.png".format(items[0]) align (0.5, 0.5) at half_size
        
        text "{}".format(item_name) size 30 align (0.5, 0.65) color "#FFF"
        # text "DEBUG: [items[0]] | [inventory_item_names]" size 20 align (0.5, 0.75) color "#FFF"
...
# Camera-specific toolbox item menu
