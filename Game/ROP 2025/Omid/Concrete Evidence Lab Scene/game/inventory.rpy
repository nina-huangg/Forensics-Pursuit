...
init python:
    ...
    def removeInventoryItemTwo(item):
        for i, sprite in enumerate(inventory_sprites):
            if sprite.type == item:
                # del inventory_sprites[i]
                inventory_sprites[i].visible = False
                inventory_sprites.pop(i)
        inventory_items.pop(inventory_items.index(item))
        repositionInventoryItems()
    """
