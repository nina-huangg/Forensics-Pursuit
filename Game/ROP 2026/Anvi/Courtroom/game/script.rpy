...
label start:
    ...
    # TODO: Replace Steve's current description of the case with your own.

    s "On June 15th, 2026, a hit-and-run collision occurred on Highway 407."
    s "A motorcyclist was struck from behind and critically injured, while the driver fled the scene before emergency services arrived." 
    s "Investigators recovered paint chip evidence and a tire tread impression from the collision scene."
    s "Surveillance footage identified several vehicles of interest, and known paint and tire samples were collected from those vehicles for comparison." 
    s "Laboratory analysis showed that Sample 2 was consistent with the recovered paint chip based on exterior color, microscopic layer sequence, and FTIR analysis of the individual paint layers."
    s "Tire tread comparison also supported Sample 2 as being associated with the scene impression."
    s "Based on the combined forensic findings, the suspected vehicle was narrowed down to a silver 2006 Dodge Durango SLT."
...
label specialty_menu:
    python:
        ...
        for item in specialties_data:
            ...
            capitalized = specialty.capitalize()
...
label tutorial_lex_diff:
    ...
    s "Great choice! On the left, you'll see the evidence button. Click this button if you want to refresh your memory about the aspects of the case you are testifying for!"
