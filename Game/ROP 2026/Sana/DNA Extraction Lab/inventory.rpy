init -10 python:
    ...
    class Evidence(Item):
        ...
        def __init__(self, name: str, image_name: str, description: str = "", usable: bool = False, action: Callable = None, collected: bool = False) -> None:
    ...
    class Instance_Item(Item):
        """
        An extension of the Item class for an item that exists within the inventory but does not primarily serve some
        functionality and represents an instance of an object.

        One motivation behind this item was that almost every other item encountered did not actually exist. What I mean by that
        is that what they were was irrelevant and that they were primarily defined by what they did. Something like fingerprint
        powder does not have meaning in that it is a representation of an object that exists, but instead an object that can do
        something.

        When trying to implement the sample tube system, this was not sufficent. A sample tube does not do much; instead, what 
        is contained within the sample or the entirity of the tube is of interest. An implication of this is that one sample tube in use 
        is meaningfully different from all others. In the context of the game, it only makes sense for a single fingeprint powder or protease K to
        exist. If an object is primarily its functionality, why on earth would we need instances? This is not the case for sample 
        tubes (e.g. two tubes needed for balancing centrifuge) and thus this subclass was required.

        This is implemented by simply including an instance attribute that refers to some kind of object.
        """
        instance: object 

        def __init__(self, name: str, image_name: str, description: str = "", usable: bool = False, action: Callable = None, instance: object = None) -> None:
            super().__init__(name, image_name, description, usable, action)
    ...
    class Inventory:
        ...
        def get_item(self, target_name: str) -> Item | None:
...
screen inventory():
    ...
    #Ensures that the inventory screen remains on top when moving between scenes.
    zorder 60
        ...
        hbox:
            ...
            if transfer_source is None:
                imagebutton:
                    ...
                    #(Notify("Disable tranfer mode by pressing the button again to access toolbox") if transfer_source is not None else SetVariable("selected_inventory", toolbox))
            ...
            if show_overlay:
                ...
                hbox:
                    ...
                    imagebutton:
                        ...
                        action (Function(attempt_transfer, item) if transfer_source is not None else [SetVariable("last_used_item", name), Function(use_item, usable, item_action)])
