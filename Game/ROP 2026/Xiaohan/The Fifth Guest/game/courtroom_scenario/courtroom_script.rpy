...
define stv = Character(name=("Steve"), image="steve", dynamic=False)
...
default voir_dire_mentioned = set()
...
label courtroom_transition_loading:
    # Counterpart to lab_transition_loading: a held beat between scenarios so the
    # jump to court does not land mid-sentence.
    ...
    window hide
    scene bg spec
    show screen courtroom_loading_overlay
    with Dissolve(1.0)
    $ renpy.pause(0.2 if dev_mode else 2.5)
    hide screen courtroom_loading_overlay

    jump courtroom_start


label courtroom_start:
    # Switch the shared say/input/inventory screens over to their courtroom look.
    $ enter_courtroom_ui()
    $ reset_courtroom_state()

    scene bg spec
    show steve normal2
    stv "Welcome to the courtroom. My name is Steve, and I'll be guiding you through your preparation before you deliver expert testimony!"
    stv "Let's reflect on the case so far."

    stv "On [CASE_DATE], [victim_name] was found deceased in the study of their home during a small gathering with four guests present."
    stv "The victim sustained fatal blunt force trauma to the head, with hemorrhage identified as the mechanism of death."
    stv "During the investigation, bloodstains were collected from the scene and biological material was recovered from beneath the victim's fingernails."
    stv "DNA analysis showed that the blood swab produced a profile consistent with [victim_name], while the under-nail swab produced a profile consistent with [suspect_name]."
    stv "The DNA association was evaluated using a Random Match Probability of approximately 1 in 269 septillion people."
    stv "A latent fingerprint recovered from the scene was identified as originating from [suspect_name], while the other three individuals were excluded as the source of the latent impression."
    stv "Now that you are familiar with the case, let's get you ready to testify as an expert witness!"
    stv "Would you like me to go through the tutorial? Or should we get straight to selecting your preferences?"
    hide steve normal2
    ...
    menu:
        ...
        "Let's do the tutorial first":
            ...
            jump court_tutorial_start
        "Let's get straight to selection":
            $ tutorial_skipped = True
            jump court_specialty_menu


label court_tutorial_start:
    hide steve normal2
    show steve normal
    stv "Don't worry, this is not a real court, and there currently are no stakes involved. It is simply a training simulation intended to help you practice!"
    hide steve normal1
    show steve normal2
    stv "Let's choose your forensic specialty."
    hide steve normal
    show steve normal2
    stv "As an expert witness, your role is to analyze and present evidence within the scope of your expertise." 
    stv "Your duty to the court is to present objective information that will help the triers of fact make a decision."
    hide steve normal2
    show steve paper
    stv "The specialty you select will determine the area of expertise and the aspects of the case that you will have to testify for. Please review your options carefully!"
    hide steve paper
    jump court_specialty_menu


label court_specialty_menu:
    python:
        # Built from specialties_list, which courtroom_data.rpy already loaded and
        # whose case-name placeholders are already resolved.
        chosen_specialty = renpy.display_menu(
            [(sp.name, sp.name) for sp in specialties_list]
        )

    call screen specialty_exploration_screen(chosen_specialty)


label court_tutorial_lex_diff:
    scene bg spec
    show steve normal
    show screen open_inv
    $ enable_evidence(get_specialty(persistent.specialty))
    stv "Great choice! On the left, you'll see the evidence button. Click this button if you want to refresh your memory about the aspects of the case you are testifying for!"
    stv "If you want to read the item description, click the info button on the item."
    stv "Now, there's just one more thing before you step into court."
    hide steve normal
    show steve normal2
    stv "Inside the courtroom, you'll be examined by Lex Machina, a mock trial lawyer."
    hide steve normal2
    show steve normal
    stv "Depending on your selection, Lex will take on one of two roles—either as the prosecution or the defense."
    hide steve normal
    show steve paper
    stv "If you choose prosecution, Lex will act as the Crown attorney. This is the easier option as you will be providing examination-in-chief." 
    stv "As a prosecutor, Lex's goal is to establish the truth and ensure your testimony is clear, credible, and useful to the court." 
    stv "In this difficulty, if you miss something important, Lex may prompt you to clarify or expand on your findings."
    hide steve paper
    show steve normal2
    stv "If you choose defense, Lex will act as the defense attorney. This is the harder option."
    hide steve normal2
    show steve normal
    stv "A defense lawyer's priority is to advocate for their client, the accused, within the bounds of the law to test the strength of the crown's case." 
    stv "They may challenge you and your findings, perhaps even working to discredit your testimony." 
    stv "Expect Lex to challenge you with leading or loaded questions, and other cross-examination techniques designed to undermine your credibility." 
    stv "You'll need to stay composed, justify your conclusions, and ensure your testimony remains admissible."
    hide steve normal
    show steve normal
    stv "Choose wisely! Your decision will shape the difficulty of your examination."
    hide steve normal
    show steve paper
    stv "Once you've made your selection, the court room awaits! I'll be seeing you after your trial. Good luck!"
    hide screen inventory
    hide screen open_inv
    jump court_difficulty_selection 


label court_difficulty_selection:
...
label court_lex_intro2:
    ...
    $ current_question = ""
    jump court_generate_voir_dire_question


label court_generate_voir_dire_question:
    # Dev mode: straight into the examination, qualified. Saves 5 questions and
    # roughly 10 Gemini calls per test run.
    if dev_mode:
        $ qualification_score = 5
        jump court_generate_first_question
    ...
    if voir_dire_question_count < 5:
        ...
        $ ai_question = generate_response(prompt_template, include_evidence_context=False)
        ...
        jump court_ask_voir_dire_question
        
    else:
        jump court_evaluate_voir_dire_result


label court_ask_voir_dire_question:
    ...
    if "thank you, let's move on" in ai_evaluation.lower():
        ...
        l "I wasn't able to evaluate that response. Please answer the question again."
        jump court_ask_voir_dire_question
    
    jump court_next_voir_dire_question


label court_next_voir_dire_question:
    $ voir_dire_question_count += 1
    jump court_generate_voir_dire_question


label court_evaluate_voir_dire_result:
    if qualification_score >= 3:
        ...
        hide navya judgement
        hide lex normal1
        jump court_game_over

        
#########################


label court_generate_first_question:
    if not first_question_generated:
        $ ai_first_question = generate_response("Generate the first question about the evidence in the case, based on the expert's chosen specialty.")
    ...
    jump court_interview_loop


label court_interview_loop:
    while True:
        show screen reminder
        if dev_mode:
            $ user_prompt = dev_auto_answer(persistent.specialty)
        else:
            $ user_prompt = renpy.input("Your answer here:")

        # Track mentioned truth bases
        python:
            # Store-level block: do not shadow `evidence` (the crime scene's Inventory).
            court_sp = get_specialty(persistent.specialty)
            for court_ev in court_sp.evidence:
                for truth in court_ev.truth_base:
        ...
        python:
            if "i have no further questions, your honour" in ai_response.lower(): 
                renpy.jump("court_interview_end")
...
label court_interview_end:
    ...
    python:
        try:
            global evidences
            case_details = evidences.get_item_by_name("Casefile")
            case_details = case_details.description
            all_truths = create_all_truths_set(persistent.specialty)
            full_transcript = "\n".join(context_history)
            evaluation_prompt = (
                f"Evaluate the expert witness testimony based on the following transcript:\n\n{full_transcript}\n\n"
                f"The expert testified as a {persistent.specialty} in {case_details}.\n\n"
    ...
    hide screen open_inv
    show steve normal
    stv "Welcome back, [player_fname]! In just a second, Lex and the Judge will return with any feedback they have for you."
    hide steve normal
    show steve normal2
    stv "After you receive your feedback, I encourage you to testify again for the [unplayed_difficulty] to get the full court room experience."
    hide steve normal2
    show steve normal
    stv "You can also choose a whole new case and specialty and start again! The choice is yours!"
    #ADD FOOTSTEPS SOUND HERE HEHEHE
    hide steve normal
    show steve normal
    stv "Oh! Sounds like Lex and the Judge are about to join us. Don't worry, I'm sure you did great!"
    hide steve normal
    jump court_evaluation_sec


label court_evaluation_sec:
    scene bg spec
    show lex normal1
    l "Hello, Steve. [player_prefix] [player_lname], are you ready to receive your evaluation?"
    hide lex normal1
    call screen evaluation_screen                                                                                

label court_ending_0:
    $ exit_courtroom_ui()
    scene bg spec
